                ...
                <div className="space-y-4">
                  {sections.filter(s => s.isActive).map((sec) => {
                    if (sec.type === 'HeroBanner') {
                      return (
                        <div key={sec.id} className="relative aspect-[4/5] bg-slate-900 text-white flex items-end overflow-hidden">
                    ...
                    if (sec.type === 'CategoryShowcase') {
                      return (
                        <div key={sec.id} className="p-3.5 space-y-2 text-left">
                          <h6 className="text-[9px] uppercase tracking-widest font-black text-slate-400">{sec.title}</h6>
                          <div className="grid grid-cols-3 gap-2">
                            {sec.categories?.map((c, i) => (
                              <div key={i} className="relative rounded aspect-square overflow-hidden bg-slate-50 flex items-center justify-center">
                    ...
                    if (sec.type === 'FeaturedProducts') {
                      return (
                        <div key={sec.id} className="p-3.5 space-y-2.5 text-left bg-slate-50">
                          <div>
                            <h6 className="text-[9px] uppercase tracking-widest font-black text-slate-400">{sec.title}</h6>
                            <p className="text-[8px] text-slate-500 leading-tight block mt-0.5">{sec.subtitle}</p>
                          </div>
                          <div className="grid grid-cols-3 gap-2">
                            {sec.productIds?.map(id => {
                    ...
                    if (sec.type === 'PromoBanner') {
                      return (
                        <div key={sec.id} className="relative py-8 px-4 text-center text-white overflow-hidden bg-slate-950" style={{ backgroundImage: `url(${sec.backgroundImage})`, backgroundSize: 'cover' }}>
            ...
            <div className="bg-white text-slate-905 h-[500px] overflow-y-auto">
              ...
              <div className="space-y-8">
                {sections.filter(s => s.isActive).map((sec) => {
                  if (sec.type === 'HeroBanner') {
                    return (
                      <div key={sec.id} className="relative h-[280px] text-white flex items-end" style={{ backgroundImage: `url(${sec.backgroundImage})`, backgroundSize: 'cover', backgroundPosition: 'center' }}>
                  ...
                  if (sec.type === 'CategoryShowcase') {
                    return (
                      <div key={sec.id} className="p-8 space-y-4 max-w-4xl mx-auto">
                        ...
                        <div className="grid grid-cols-3 gap-6">
                          {sec.categories?.map((c, i) => (
                            <div key={i} className="relative rounded-xl overflow-hidden aspect-[4/5] bg-slate-100 flex items-end p-4">
                  ...
                  if (sec.type === 'FeaturedProducts') {
                    return (
                      <div key={sec.id} className="p-8 space-y-4 bg-slate-50">
                        ...
                        <div className="grid grid-cols-3 gap-6 max-w-4xl mx-auto pt-2">
                          {sec.productIds?.map(id => {
                            ...
                            if (!pr) return null;
                            return (
                              <div key={pr.id} className="space-y-1.5 bg-white border p-3 rounded-xl shadow-sm">
                  ...
                  if (sec.type === 'PromoBanner') {
                    return (
                      <div key={sec.id} className="relative py-12 px-8 text-center text-white" style={{ backgroundImage: `url(${sec.backgroundImage})`, backgroundSize: 'cover', backgroundPosition: 'center' }}>
                        <div className="absolute inset-0 bg-slate-950/70"></div>
